          ...
          if(ProjId.size()>0){
           ...
           if (trigger.isBefore && Trigger.isInsert) {
                ...
                for(opportunity o : trigger.new){
                   ...
                   o.stagename='Opportunity Created';           
                   Id Currentprojid = o.Project_Name__c;
                   if(Currentprojid != null){
                       Project__c p = [select id,Stage__c from Project__c where id=:Currentprojid];
                       
                   if(p.Stage__c=='Project Won - Order Paid'){
                    o.addError('You cannot create an opportunity under a project with stage project won.');
                   }
    ...
    /**************************************************************************************************************/
        ...
        if (Trigger.isAfter && (Trigger.isInsert || Trigger.isUpdate)) {
            ...
            //if (!UtilityClass.updateFromQuoteTrigger) 
                for (Opportunity opp: Trigger.new) {
                   ...
                   // if (opp.StageName == UtilityClass.opportunityWonStatus && opp.createdDate > mydate && trigger.oldMap != null && trigger.oldMap.get(opp.Id).StageName != UtilityClass.opportunityWonStatus) 
                    ...
                    if (opp.StageName != NULL && opp.stageName !='Closed/Lost' && opp.stageName !='Closed/Inactivity' && opp.stageName != 'On Hold' && opp.stageName !=UtilityClass.opportunityWonStatus && opp.stageName !='Bid Unsuccessful' && opp.stageName !='Cancelled'  && trigger.oldMap!=null && ((trigger.oldMap.get(opp.Id).StageName != opp.StageName)) && opp.stageName!='Influencer Closed/Won' ) {
                        projIds2.add(opp.Project_Name__c);
                        system.debug('Praveen : I am here' + utilityClass.runStageUpdateOnce + projIds2);
                    }
                }
            ...
            //if (!UtilityClass.updateFromQuoteTrigger) 
                ...
                if (projIds2.size() > 0 && utilityClass.runStageUpdateOnce ) { // new boolean used 
                    //Call method from OpportunityHelper Utility class
                    system.debug('PRAVEEN : I am here2');
