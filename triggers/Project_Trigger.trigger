...
trigger Project_Trigger on Project__c (before insert, before update,after insert, after update) {
    ...
    *************************************************************************************************/
    ...
    /**************************************************************************************************************/
    /************************************************************************************************  
     Trigger on After Update for Project Cancelled and Onhold    
    *************************************************************************************************/
    if(trigger.isAfter && trigger.isUpdate){
      set<Id> Projids = new set<Id>();
      for(project__c p : trigger.new){
      if(p.stage__c=='Project Lost' || p.stage__c=='Project Cancelled' || p.stage__c=='Project Inactive' || p.stage__c=='Project On Hold'){
         Projids.add(p.id);
      ...
      ProjectHelper.updateOpportunityonHoldandCancelled(Projids);
